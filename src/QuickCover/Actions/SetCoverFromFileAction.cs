...
{
    public class SetCoverFromFileAction
    {
        ...
        public SetCoverFromFileAction(IPlayniteAPI playniteApi, ImageImportService imageImportService)
        {
        ...
        public void Execute(IEnumerable<Game> games)
        {
            ...
            try
            {
                imageImportService.SetCoverImage(game, imagePath);
                ShowNotification($"Cover updated for {game.Name}.", NotificationType.Info);
            }
            catch (Exception exception)
            {
                ShowNotification($"Failed to update cover: {exception.Message}", NotificationType.Error);
            }
        }
